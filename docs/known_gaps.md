...
- File management now has a dedicated frontend route/page wired to the implemented file metadata list/detail/delete-invalidate APIs. Upload, download, preview, S3-compatible storage configuration UI, and file-reference listing remain incomplete because their concrete backend endpoints/contracts are not implemented in this slice.
