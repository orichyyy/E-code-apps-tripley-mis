...
export function NotificationTemplateStatusBadge({
  status,
}: {
  ...
  return (
    <section className="rounded-lg border bg-card p-4 text-sm shadow-sm">
      ...
      <p className="mt-2 text-muted-foreground">
        Manage persisted in-app, email, and reserved SMS template records. Delivery drivers and
        outbound sending remain optional integrations outside this page.
      </p>
      <div className="mt-4 rounded-md border bg-muted/30 p-3 text-xs text-muted-foreground">
        Variables are stored as a list of supported placeholders and are edited as comma-separated
        names.
      </div>
    </section>
  );
