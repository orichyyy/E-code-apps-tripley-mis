...
export function ProfilePage() {
  ...
  if (query.isLoading) {
    return (
      <ProfileState icon={<Loader2 className="size-5 animate-spin" />} title="Loading profile" />
    );
  }
          ...
          children={(field) => (
            <div className="md:col-span-2">
              ...
              <Button
                ...
                disabled={busy}
                onClick={() =>
                  onAvatarSubmit(field.state.value.trim() ? field.state.value.trim() : null)
                }
                size="sm"
                type="button"
                variant="outline"
...
function ReadonlyField({ label, value }: { label: string; value: string }) {
  return (
    <label className="grid gap-2 text-sm font-medium text-muted-foreground">
      {label}
      <input
        className="h-10 rounded-md border bg-muted px-3 text-foreground"
        readOnly
        value={value}
      />
    </label>
