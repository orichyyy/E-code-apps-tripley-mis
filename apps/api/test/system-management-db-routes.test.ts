...
describe("database-backed system management routes", () => {
  it.runIf(postgresqlUrl)("persists system config, dictionary, and i18n mutations in PostgreSQL", async () => {
...
async function getI18nMessageId(executor: DatabaseAdapterExecutor, messageKey: string): Promise<string> {
  const rows = await executor.all("SELECT id FROM i18n_messages WHERE message_key = $1", [messageKey]);
