...
describe("communications routes", () => {
  it("exposes announcement and webhook subscription APIs through authenticated base permissions", async () => {
    ...
    const updateAnnouncementResponse = await app.request(`/api/announcements/${announcementBody.data.id}`, {
      method: "PATCH",
      headers,
    ...
    const unpublishResponse = await app.request(`/api/announcements/${announcementBody.data.id}/unpublish`, {
      method: "POST",
      headers
    });
    ...
    await expect(unpublishResponse.json()).resolves.toEqual({
      data: expect.objectContaining({ id: announcementBody.data.id, status: "draft", publishedAt: null })
    });
    ...
    await expect(updateWebhookResponse.json()).resolves.toEqual({
      data: expect.objectContaining({ id: webhookBody.data.id, status: "disabled", secretConfigured: true })
    });
