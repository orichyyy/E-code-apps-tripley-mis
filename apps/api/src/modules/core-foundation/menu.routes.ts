...
export function createMenuRoutes(services: BackendCoreServices) {
  ...
  routes.post("/menus", async (context) => {
    ...
    return context.json({ data: services.createMenu(input) }, 201);
  });

  routes.patch("/menus/:id", async (context) => {
    const input = updateMenuRequestSchema.parse(await context.req.json());
    return context.json({ data: services.updateMenu(context.req.param("id"), input) });
  });

  routes.delete("/menus/:id", (context) => {
    return context.json({ data: services.deleteMenu(context.req.param("id")) });
  });
