...
export function createWebhookDeliveryProcessor(options: {
  ...
  return {
    async fanOutPending() {
      ...
      return options.repository.fanOutPending(
        options.config.eventSource,
...
async function deliverOne(
  ...
  try {
    const envelope = createCloudEventEnvelope(
      delivery.eventId,
...
function resultFromResponse(
  ...
  return failedResult(
    ...
    `Webhook receiver returned HTTP ${response.statusCode}.`,
    retry,
    response.statusCode,
...
function failedResult(
  ...
  return {
    ...
    nextAttemptAt: retry ? (nextAttemptAt ?? nextAttempt(delivery.attempt)) : null,
  };
}
...
function nextAttempt(attempt: number, retryAfter: string | null = null, status = 0): string {
  ...
  const seconds =
    retryAfterMs ?? delays[Math.min(Math.max(attempt - 1, 0), delays.length - 1)] ?? 1800;
...
function safeLog(delivery: ClaimedWebhookDelivery, result: WebhookAttemptResult) {
  ...
  try {
    targetHost = new URL(delivery.targetUrl).hostname;
  } catch {
    /* safe fallback */
  }
  return {
