...
export function createDatabaseRateLimitAdapter(executor: DatabaseAdapterExecutor): RateLimitAdapter {
  return {
    async check(key, limit, windowSeconds) {
      return executor.transaction(async () => checkWithinTransaction(executor, key, limit, windowSeconds));
    },
    async healthCheck() {
...
async function checkWithinTransaction(
  ...
  const windowStartsAt = new Date(Math.floor(nowMs / (windowSeconds * 1000)) * windowSeconds * 1000).toISOString();
